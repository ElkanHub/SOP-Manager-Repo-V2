...
> **Version:** 3.10 (PWA + Offline-First Messaging)
> **Current Phase:** Phase 43 ✅ Complete — Phase 44 Next
...
| Phase 43 | ✅ Complete | PWA Foundation + Offline-First Messaging         |
...
## Phase 43: PWA Foundation + Offline-First Messaging

> **Goal:** Make SOP-Guard Pro installable on phones, tablets, and desktops, render previously-visited pages with no network, and let users keep sending messages even when the WebSocket drops. Build is now `next build --webpack` because `@serwist/next` does not yet support Turbopack — Next 16 dev still runs on Turbopack and the service worker is disabled in dev anyway.

### Completed Tasks

**Installable shell**

- `app/manifest.ts` — Next App-Router manifest at `/manifest.webmanifest`. Brand-navy theme, `standalone` display, `start_url: /dashboard`, three icons (192, 512, 512 maskable). iOS `appleWebApp` + `viewport.themeColor` set in the root layout.
- `public/icons/` — 9 PNG sizes (72/96/128/144/152/180/192/384/512) plus `apple-touch-icon.png` and a maskable 512. Generated by `scripts/generate-pwa-icons.mjs` from a navy "SG" SVG. **Placeholders — replace with real artwork before launch.**
- `proxy.ts` — short-circuits `/manifest.webmanifest`, `/sw.js`, `/sw.js.map`, `/icons/*`, `/offline`, and `/swe-worker-*` before the auth check. Without this, Chrome's anonymous fetch of the manifest got redirected to `/login` and was rejected with "Manifest: Line 1, column 1, Syntax error" — which silently suppressed `beforeinstallprompt` everywhere.

**Service worker (Serwist)**

- `app/sw.ts` — runtime caching split by surface:
  - CacheFirst: static assets, `/_next/static/*`, Supabase Storage, Google Fonts
  - NetworkFirst (5 s timeout): page navigation, Supabase REST GETs
  - NetworkOnly: `/api/auth`, `/api/cron`, all non-GET requests
- `fallbacks` registers `/offline` as the document fallback for un-cached navigations.
- `next.config.ts` wraps the config with `withSerwistInit({ swSrc: 'app/sw.ts', swDest: 'public/sw.js', disable: process.env.NODE_ENV === 'development' })`.
- `tsconfig.sw.json` gives the worker its own WebWorker lib; root `tsconfig.json` excludes `app/sw.ts` so `tsc --noEmit` doesn't choke on `self.skipWaiting`.

**Offline data layer**

- `lib/db/offline-db.ts` — IndexedDB (`sop-guard-offline`, v2) with stores: `sops`, `equipment`, `departments`, `profiles`, `messages_cache`, `sync_queue`, `meta`.
- `lib/db/offline-records.ts` — generic get/put/seed/delete helpers operating on `OfflineRecord<T>` envelopes (`id`, `data`, `updated_at`, `synced`, `deleted`).
- `lib/db/sync-queue.ts` — `enqueueSync`, `getPendingSyncItems`, `markSyncFailure`, retry up to 5 then abandon.
- `lib/db/messages-cache.ts` — per-conversation message cache with `by-conversation` index, `cachePendingMessage` / `replacePendingMessage` for temp-id swapping.
- `hooks/use-offline-seed.ts` — on mount and on `online` event, hydrates the four reference stores from Supabase. Throttled to once per five minutes via the `meta` store.

**Sync engine**

- `lib/sync/sync-engine.ts` — drains the queue in `created_at` order. Last-Write-Wins on UPDATEs via `.lt('updated_at', payload.updated_at)` — server keeps its row if it's newer. Direct REST for tables with INSERT/UPDATE/DELETE RLS policies.
- `lib/sync/handlers.ts` — a handler registry keyed by `${table}:${action}`. Items with a `handler` field bypass the direct-REST path and dispatch to a registered function (used for tables guarded by service-role server actions).
- `lib/sync/message-handlers.ts` — registers `messages:INSERT` to call the existing `sendMessage()` server action so RLS membership checks and audit logging stay intact rather than being bypassed by a direct anon write.
- `hooks/use-sync-on-reconnect.ts` — runs the sync engine on mount and on every `online` event, invalidates all TanStack Query keys on success, toasts how many items were synced or abandoned.

**Offline-aware UI**

- `hooks/use-online-status.ts` + `components/offline-banner.tsx` — sticky amber banner across the top of the dashboard whenever `navigator.onLine === false`.
- `components/sync-status.tsx` — small inline pill ("Offline · 3 pending" / "Syncing 2…"). Polls IndexedDB every 10 s.
- `components/pwa/pwa-provider.tsx` — mounted in the dashboard layout. Runs `useOfflineSeed`, `useSyncOnReconnect`, `useSwUpdate`, and registers the message sync handler exactly once.

**Install prompts**

- `hooks/use-install-prompt.ts` — global capture of `beforeinstallprompt` multicast to subscribers, so the auto-toast and a manual button share the same event instead of racing for it. Detects iOS Safari (including iPadOS reporting as Mac) and excludes in-app browsers.
- `components/pwa/install-prompt.tsx` — toast appears once the event is captured, suppressed on `/` and `/contact`, dismissed for 14 days via `localStorage.pwa_install_dismissed_at`. iOS fallback toast tells the user to tap Share → "Add to Home Screen".
- `components/pwa/install-button.tsx` — Download icon in the top nav whenever the app is installable, Share icon on iOS. Lets users install without waiting for Chrome's engagement heuristics to fire the auto-toast.

**In-app update flow**

- `app/sw.ts` — `skipWaiting: false`. New workers install and wait; activation is gated on a `SKIP_WAITING` message from the window.
- `hooks/use-sw-update.ts` — on mount calls `registration.update()` immediately, listens for `updatefound`, polls every 30 minutes for long-lived tabs, re-checks on tab focus / visibility. When a waiting worker is detected, fires a Sonner toast: "New version available — Reload / Later". Clicking Reload posts `SKIP_WAITING`, waits for `controllerchange`, then refreshes.
- `next.config.ts` — `/sw.js` and `/manifest.webmanifest` now return `Cache-Control: public, max-age=0, must-revalidate`. Without this Vercel served `/sw.js` with default static caching, so the browser never re-fetched it and the toast never had a chance to fire.

**Offline-first messaging (Phase 43.2 — converted Messages from connected-only)**

- `components/messages/conversation-thread.tsx` — three behaviour changes:
  1. **Load:** hydrate from `messages_cache` *before* the network fetch so the thread renders instantly even with no signal. Merge the fresh server batch in once it arrives; preserve any still-pending temp messages whose real version hasn't shown up yet.
  2. **Send:** persist the optimistic message to IndexedDB via `cachePendingMessage`, attempt `sendMessage()` directly when online, and on any failure / `!navigator.onLine` queue the payload via `enqueueSync('messages', 'INSERT', …, { handler: 'messages:INSERT' })`. The bubble renders a "Sending…" pill with a pulsing dot until confirmed.
  3. **Reconnect:** on `online` event, re-run `loadThread()` so any messages the WebSocket missed during the disconnect are backfilled. `runSync()` runs first to drain any queued sends.
- Realtime INSERT handler also persists incoming messages to `messages_cache` so reopening a thread offline shows the full history, and now calls `replacePendingMessage` when it dedups an optimistic temp message so the cache doesn't leak duplicates.

**Auto-scroll fix in the thread (long-standing UX bug)**

- Conversation now scrolls to the latest message on open instead of stranding the user at the top. Uses a container ref + `scrollTo({ top: scrollHeight })` after two `requestAnimationFrame`s so layout has settled.
- Follows new messages only when the user is already within ~150 px of the bottom, so reading history doesn't get yanked.

### Files Touched

```
app/manifest.ts                                          # NEW — installable manifest
app/sw.ts                                                # NEW — Serwist runtime caching + skip-waiting message handler
app/offline/page.tsx                                     # NEW — offline fallback page
app/layout.tsx                                           # iOS appleWebApp meta + viewport themeColor + InstallPrompt mount
app/(dashboard)/layout.tsx                               # PwaProvider wraps the shell
next.config.ts                                           # withSerwist wrapper + sw.js/manifest cache headers
proxy.ts                                                 # Exempt manifest / sw / icons / offline from auth redirect
tsconfig.json                                            # Exclude app/sw.ts from main project
tsconfig.sw.json                                         # NEW — WebWorker lib for the worker
package.json                                             # build script switched to `next build --webpack`
.gitignore                                               # Ignore generated /public/sw.js + worker chunks

components/pwa/pwa-provider.tsx                          # NEW — mounts hooks + banner inside dashboard
components/pwa/install-prompt.tsx                        # NEW — Chromium + iOS install toasts
components/pwa/install-button.tsx                        # NEW — top-nav install button
components/offline-banner.tsx                            # NEW — sticky offline banner
components/sync-status.tsx                               # NEW — pending-sync indicator
components/shell/top-nav.tsx                             # Wire InstallButton

hooks/use-online-status.ts                               # NEW
hooks/use-offline-seed.ts                                # NEW — hydrate IndexedDB from Supabase
hooks/use-sync-on-reconnect.ts                           # NEW — drain queue on connect
hooks/use-install-prompt.ts                              # NEW — shared beforeinstallprompt capture
hooks/use-sw-update.ts                                   # NEW — SW update detection + toast

lib/db/offline-db.ts                                     # NEW — IndexedDB schema v2
lib/db/offline-records.ts                                # NEW — generic record helpers
lib/db/sync-queue.ts                                     # NEW — write queue
lib/db/messages-cache.ts                                 # NEW — per-conversation message cache
lib/sync/sync-engine.ts                                  # NEW — LWW queue drain + handler dispatch
lib/sync/handlers.ts                                     # NEW — handler registry
lib/sync/message-handlers.ts                             # NEW — messages:INSERT → sendMessage action

components/messages/conversation-thread.tsx              # Cache hydration, queued sends, reconnect refetch, auto-scroll fix

public/icons/*.png                                       # NEW — 11 placeholder icons
scripts/generate-pwa-icons.mjs                           # NEW — one-shot icon generator using sharp
PWA_IMPLEMENTATION_PLAN.md                               # NEW — implementation spec (root)
```

### Verification

- `npx tsc --noEmit` and `npx tsc --noEmit -p tsconfig.sw.json` both pass ✅
- `next build --webpack` succeeds; `/sw.js` is emitted (~53 KB) and `/manifest.webmanifest` is listed as a static route ✅
- Chrome DevTools → Application → Manifest renders the manifest without errors ✅ (was failing before the proxy fix)
- DevTools → Application → Service Workers shows `sw.js` as "activated and running" on the deployed URL ✅
- Toggling DevTools "Offline":
  - Previously-visited pages still load from cache ✅
  - Unvisited pages render `/offline` ✅
  - Messages thread renders cached messages, banner shows "Working offline" ✅
  - Sending a message while offline queues it (`sync_queue` row in IndexedDB) and shows "Message queued — will send when you reconnect" ✅
  - Toggling back online drains the queue, swaps the temp bubble for the real one, and invalidates queries ✅
- Install prompt:
  - Chromium fires `beforeinstallprompt` after the user satisfies engagement → both the auto-toast and the Download icon in the top nav activate ✅
  - iOS Safari shows the "tap Share → Add to Home Screen" toast and the Share icon in the top nav ✅
  - Already-installed sessions (`display-mode: standalone`) hide both surfaces ✅
- Update flow:
  - Deploying a new version triggers the update toast in open tabs within ≤30 min (and immediately on tab focus). Clicking Reload swaps in the new worker and refreshes the page ✅

### Known Limits / Follow-Up

- **PWA icons are placeholders.** Drop a real 1024×1024 logo into `public/icons/source.png` (or hand to a designer) then re-run `node scripts/generate-pwa-icons.mjs`.
- **Local-first writes for other domains aren't wired yet.** SOPs, equipment, approvals, change control, PM tasks, training, requests, calendar events still go straight to the server. The plumbing is in place — each feature needs the `cachePendingX` → `enqueueSync` pattern that messages now use. Messages was first because it's the highest-traffic surface most likely to be used on spotty connections.
- **Messages: edits and deletes while offline are not queued.** They fail silently if the user is offline. Adding `messages:UPDATE` and `messages:DELETE` handlers is straightforward when needed.
- **Inbox list (`/messages`) is not cached.** Individual threads are. So an offline user can re-open a thread they previously viewed but the conversation list itself will look empty.
- **`@serwist/next` does not support Turbopack.** Production builds use webpack (`next build --webpack`); dev still uses Turbopack and the SW is disabled there anyway. If Turbopack production becomes a priority, the migration path is `@serwist/turbopack` (experimental).
- **Departments table has no `updated_at` column.** No client-driven writes flow through the LWW path today, so this is fine, but worth noting if departments ever become user-editable from the app.
