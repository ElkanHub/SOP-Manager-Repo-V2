...
> **AI Agent Instruction File** | Version 2.2 | Star-Centralized Architecture + Messaging
...
- The appshould have light mode dark mode and system mode for MVP. The dark mode should not just be a seperate colour pallete but should be a normal gray dark mode with blue and teal accents.
...
### Git Discipline
- Leave all commits to the Developer after each phase. The Developer will then review the work and then give the go ahead to build the next phase.
...
An industrial SaaS platform for managing Standard Operating Procedures (SOPs) and Preventive Maintenance (PM). Built on a **star-centralized architecture**: the SOP Library and Equipment Registry are the two central hubs. Everything else — approvals, change control, PM planner, calendar, pulse, reports, messaging — orbits them. There are no department pages, no department navigation, no tree-node structure.

The platform also includes a **built-in Messaging system** (Phase 13) — direct messages and group conversations between team members, with the ability to reference any in-app record (SOP, equipment, change control) inline inside a message. File uploads are deferred to a future version. Every conversation is personal: each user sees only the conversations they are a member of, enforced at the RLS level.
...
/app
  ...
  /reports/page.tsx
    /settings/page.tsx
    /messages/page.tsx                 ← Messaging hub (Phase 13)
    /messages/[id]/page.tsx            ← Individual conversation (Phase 13)
/api
...
type             text        NOT NULL
                 CHECK (type IN (
                   ...
                   'system',
                   'todo',
                   'message'           ← notification that a chat message arrived
                 ))
title            text        NOT NULL
body             text
entity_type      text
...
| `update_conversation_last_message()` | trigger | Fires on messages INSERT. Updates conversations.last_message_at and last_message_body (truncated to 60 chars). |
| `notify_message_pulse()` | trigger | Fires on messages INSERT. Checks each conversation_member's notify_setting. Inserts pulse_items row for each member who should be notified (recipient_id = user_id, type = 'message', entity_type = 'conversation', entity_id = conversation_id). Skips: sender, muted members, mentions_only members not in message.mentions. |
...
| profiles | SELECT | Any active user (limited) | `is_active_user(auth.uid())` — returns only id, full_name, avatar_url, department, role for people search in messaging. This is a deliberate, controlled policy. |
...
| conversations | SELECT | Member | `auth.uid() IN (SELECT user_id FROM conversation_members WHERE conversation_id = id)` |
| conversations | INSERT | Any active user | `is_active_user(auth.uid())` |
| conversations | UPDATE | Creator | `created_by = auth.uid()` (group rename only) |
| conversations | DELETE | Nobody | No delete policy. Archive via is_archived flag. |
| conversation_members | SELECT | Member | Must be a member of that conversation |
| conversation_members | INSERT | Creator | At conversation creation. Members can add others to group conversations only. |
| conversation_members | UPDATE | Self | `user_id = auth.uid()` — only last_read_at and notify_setting columns |
| conversation_members | DELETE | Self | `user_id = auth.uid()` — leaving a group only. Cannot leave a DM. |
| messages | SELECT | Member | Must be a member of messages.conversation_id via conversation_members |
| messages | INSERT | Member | Must be a member of messages.conversation_id |
| messages | UPDATE | Sender | `sender_id = auth.uid()` — server action enforces 15-minute edit window |
| messages | DELETE | Nobody | Soft-delete only via server action (sets deleted_at) |
...
Messaging launched:    [fill in after Phase 13]
...
> These four tables are added in Phase 13 migration `014_messaging.sql`. They do not touch any existing tables.
...
last_message_at     timestamptz           ← denormalised, updated by trigger on messages INSERT
last_message_body   text                  ← preview, max 60 chars, updated by same trigger
is_archived         boolean     NOT NULL DEFAULT false
created_at          timestamptz DEFAULT now()
```
> Conversations are never hard-deleted. `is_archived = true` hides them from the list. The row and all messages remain in the DB.

### conversation_members
```sql
conversation_id   uuid  NOT NULL REFERENCES conversations
user_id           uuid  NOT NULL REFERENCES profiles
last_read_at      timestamptz DEFAULT now()   ← unread count = messages.created_at > last_read_at
notify_setting    text  NOT NULL DEFAULT 'all'
                        CHECK (notify_setting IN ('all','mentions_only','muted'))
joined_at         timestamptz DEFAULT now()
PRIMARY KEY (conversation_id, user_id)
```
> `notify_setting` controls whether a `pulse_items` row is inserted when a new message arrives in this conversation. Direct conversations always behave as `'all'` regardless of this setting — it only applies to group conversations.
...
mentions          uuid[]      NOT NULL DEFAULT '{}'
                              ← array of mentioned user_ids. Populated by server action on parse.
reference_type    text        CHECK (reference_type IN ('sop','equipment','change_control'))
reference_id      uuid                  ← FK to the referenced record. Nullable.
reply_to_id       uuid        REFERENCES messages   ← nullable. Inline quote-reply.
is_edited         boolean     NOT NULL DEFAULT false
edited_at         timestamptz           ← set by server action on edit. Null if never edited.
deleted_at        timestamptz           ← soft-delete. Body is hidden, row stays.
created_at        timestamptz DEFAULT now()
```
> **Full-text search index (must be in migration):**
...
>
> The 15-minute edit window is enforced in the server action: `IF now() - created_at > interval '15 minutes' THEN RAISE EXCEPTION 'Edit window closed'; END IF;`
>
> Deleted messages: `deleted_at` is set, body is replaced client-side with "This message was deleted." The `reference_id` card is also hidden for deleted messages.

---

## PHASE 13 — Messaging

**Goal:** Users can send direct messages to any colleague and create group conversations. Messages support `@mentions`, inline in-app references (SOP, equipment, change control), and real-time delivery via Supabase Realtime. Typing indicators work. The Pulse notifies users of new messages with a deep link into the conversation. Each user's conversation list is entirely personal and RLS-enforced.

**When to build this phase:** After Phase 12 is complete and the core platform is stable on production. Do not start Phase 13 while Phase 0–12 are in progress.

---

**Steps:**

**Step 1 — Migration `014_messaging.sql`**

Create the four messaging tables in order: `conversations`, `conversation_members`, `messages`.

Create the full-text search index and the performance index on `messages`:
```sql
CREATE INDEX messages_body_search_idx ON messages USING GIN(body_search);
CREATE INDEX messages_conversation_created_idx ON messages(conversation_id, created_at DESC);
```

Create the `update_conversation_last_message()` trigger function and attach it to `messages` AFTER INSERT:
```sql
...
Create the `notify_message_pulse()` trigger function and attach it to `messages` AFTER INSERT:
```sql
...
BEGIN
  FOR member IN
    ...
    WHERE cm.conversation_id = NEW.conversation_id
      AND cm.user_id != NEW.sender_id   -- never notify the sender
  LOOP
    -- DMs always notify. Groups respect notify_setting.
...
Add RLS to all three tables. Enable RLS first:
```sql
...
Write all RLS policies per the RLS POLICY REFERENCE table (messaging rows). The critical pattern — used on both `conversations` and `messages` SELECT — is membership verification:
```sql
...
Add the limited profiles SELECT policy for people search (messaging new conversation flow):
```sql
-- Any active user can search basic profile info to start a conversation
-- Returns only: id, full_name, avatar_url, department, role
-- This is a deliberate, documented relaxation. See SYSTEM REFERENCE — Critical Principles.
CREATE POLICY "active users can search profiles for messaging"
ON profiles FOR SELECT
USING (is_active_user(auth.uid()) AND is_active = true);
```
> **Note:** This policy allows any active user to SELECT any active profile row. Only expose the minimum fields needed in the people-search query (id, full_name, avatar_url, department, role). Do not SELECT sensitive fields (signature_url, notification_prefs, etc.) in the people-search component.
...
Add messaging TypeScript interfaces to `/types/app.types.ts`:
```typescript
...
interface MessageReference {
  ...
  id:           string;
  title:        string;      // SOP title, equipment name, or CC reference
  status:       string;      // current status of the referenced record
  department?:  string;
  version?:     string;      // SOP version if applicable
}
```
...
Add "Messages" nav item to `<Sidebar />` between Equipment and Calendar:
- Icon: `MessageSquare` (Lucide), 18px
- Label: "Messages"
- Route: `/messages`
- Unread badge: shows total unread message count across all conversations. Queried from `conversation_members` where `user_id = auth.uid()` — count conversations where `last_message_at > last_read_at`. Refreshed on Realtime `pulse_items` INSERT with `type = 'message'`.
- Active state: same as other nav items (`bg-blue-50`, `border-l-[3px] border-brand-teal`)

Add `MessageSquare` to the Pulse item renderer map with `type = 'message'`:
```typescript
message: MessagePulseItem,  // shows sender name + message preview + "Open conversation" link
```

---

**Step 3 — `/messages/page.tsx` — Conversation List**

Build the two-panel messages layout. On mobile (< 768px): show conversation list only. Selecting a conversation navigates to `/messages/[id]`. On desktop (≥ 768px): both panels visible side by side.

**Left panel (280px, fixed height, flex column):**

Header: "Messages" 15px/600 + "New" button (icon=Plus, secondary sm) → opens `<NewConversationModal />`

Search input: below header, full-width, h-32, placeholder "Search conversations..." icon=Search left-inset. Filters the conversation list client-side by name or last message preview.
...
Each conversation row (height: 64px, px-4 py-3, hover bg-surface-raised, cursor-pointer):
```
flex gap-3 items-center
[Avatar or GroupAvatar]    [Content flex-1 min-w-0]    [Meta]

Content:
  Name: 13px/600 text-primary truncate
    Direct: other person's full name
    Group: conversation.name
  Preview: 12px text-secondary truncate mt-0.5
    If last_message_body: preview text
    Else: "No messages yet" text-muted italic

Meta (flex-shrink-0 flex flex-col items-end gap-1):
  Time: 11px text-muted (time-ago if today, date if older)
  Unread badge: if unread > 0: rounded-full bg-brand-teal text-white text-10 px-1.5 min-w-[18px]
  Muted indicator: if notify_setting = 'muted': BellOff 11px text-muted
```

Active conversation row: `bg-blue-50 border-l-[3px] border-brand-teal`

**Group avatar** `<GroupAvatar members[] />`: stack of 3 small avatars overlapping (ring-2 ring-white), or 2 + a "+N" circle. Size: 36px total footprint.
...
/>
```
When conversation selected: renders `<ConversationThread />` (see Step 4).

---

**Step 4 — `<ConversationThread />`**

The active conversation view. Built as a standalone component rendered in the right panel of the messages page.

**Thread header (48px, border-bottom, flex items-center gap-3 px-4):**
- Avatar (or GroupAvatar for groups)
- Conversation name: 14px/600 text-primary
- Members subtitle for groups: "Alice, John, +3 others" 12px text-secondary
- Actions (right, flex gap-1):
  - Group only: `UserPlus` icon button → opens add member modal
  - Group only: `Settings` icon button → opens conversation settings (rename, notify setting, leave)
  - `MoreHorizontal` → kebab: Archive, Mute/Unmute

**Typing indicator bar (below header, 24px):**
```typescript
// Supabase Realtime presence channel — one per open conversation
...
// Broadcast typing when user is typing (debounced — stop after 2s inactivity)
presenceChannel.track({ user_id: user.id, name: user.full_name, is_typing: true });
```
Display: "[Name] is typing..." 12px text-muted italic. Hidden when no one is typing.

**Message area (flex-1 overflow-y-auto, flex-col-reverse for bottom-anchored scroll):**
...
supabase.from('messages')
  ...
  .limit(50)
```
Results reversed for display (show oldest at top, newest at bottom). Infinite scroll: when user scrolls to top, fetch next 50 older messages.
...
On new message received (not sent by self): update `conversation_members.last_read_at` to `now()` if the thread is currently open and focused.

**Date separators:** Group messages by date. Insert a date separator between message groups from different days:
```
── Monday, 10 March ──    (12px text-muted, horizontal rules each side)
```

**Message rendering — each message:**

Consecutive messages from the same sender within 2 minutes: grouped. Only the first shows the avatar and name; subsequent ones are indented with no avatar.

Non-deleted message:
```
flex gap-3 px-4 py-1 hover:bg-surface-raised group

[Avatar sm — shown only on first in group]
[Content]
  Sender name + timestamp (first in group only):
    14px/600 text-primary + 11px text-muted ml-2
  
  @mention highlighting: any `@[name]` token in body renders as:
    bg-brand-teal/15 text-brand-teal rounded-sm px-1 font-600 text-13
    If mention is current user: bg-brand-teal/25 (brighter highlight)
  
  Body: 14px text-primary leading-relaxed

  Reply quote (if reply_to_id set — inline above body):
    bg-surface-raised border-l-2 border-brand-teal rounded-r-md px-3 py-1.5 mb-1.5
    "[Sender name]" 11px/600 text-muted + body preview 12px text-secondary truncate
    Clicking scrolls to the original message and briefly highlights it

  Reference card (if reference_type is set — below body):
    bg-surface-raised border border-border-default rounded-lg px-4 py-3
    border-l-4 border-brand-teal
    flex items-start gap-3
      [icon: FileText / Cog / GitBranch — 18px text-brand-teal]
      [Content]
        Type label: "SOP" / "Equipment" / "Change Control" — 11px uppercase text-muted
        Title: 13px/600 text-primary mt-0.5
        Row: <StatusBadge size="sm" /> + version (DM Mono 11px) + <DeptBadge size="sm" />
      [right: "Open →" ghost button sm]
    
    If referenced record is superseded/inactive:
      Overlay badge: "Superseded" or "Inactive" in the top-right of the card
      "Open →" button still works — opens with current status shown

  Edited label (if is_edited): "edited" 11px text-muted italic after body

  Hover actions (appear on message row hover, position absolute right):
    bg-white border border-border-default rounded-md shadow-sm flex items-center gap-1 px-1.5
    Reply (Reply icon) · Mention (AtSign icon) · Edit (Pencil — own messages, within 15 min) · Delete (Trash — own messages)
```

Deleted message:
```
flex gap-3 px-4 py-1 opacity-60
[Avatar sm placeholder]
"This message was deleted." — 13px text-muted italic
```

**Input bar (flex-shrink-0, border-top, px-4 py-3):**

Reply preview (shown above input when replying):
```
bg-surface-raised border-l-2 border-brand-teal rounded-r-md px-3 py-2 mb-2
flex items-center justify-between
"Replying to [name]" 12px/600 text-brand-teal + message preview 12px text-secondary truncate
X button (right): clears reply state
```

Mention suggestions popover (appears above input when `@` typed):
```
Position: absolute above textarea, left-aligned with input
bg-white border border-border-default rounded-lg shadow-lg max-h-[200px] overflow-y-auto
Each row: flex items-center gap-3 px-4 py-2.5 hover:bg-surface-raised cursor-pointer
  <Avatar sm /> + full name 13px/600 + dept badge sm
Filter: as user types after `@`, filter conversation members by name prefix
Select: inserts `@[full_name]` into textarea, adds user_id to mentions array
```
...
[Reference button] icon=Paperclip 20px, ghost, h-36, opens <ReferencePicker />
[Textarea] flex-1 min-h-[36px] max-h-[120px] auto-resize
           placeholder "Message [name]..."
           On @ type: show mention suggestions
[Send button] primary, h-36, icon=Send, disabled when body is empty or whitespace only
              Keyboard: Enter sends. Shift+Enter inserts newline.
```

**`<ReferencePicker />`** (Shadcn Popover, opens above the input bar):
```
Width: 360px, max-height: 400px overflow-y-auto
Search input at top: "Search SOPs, Equipment, Change Controls..."

Three sections (shown when search is empty — browse mode):
  Recent SOPs    (last 5 from user's Library)
  Recent Equipment (last 5 from user's dept)
  Open Change Controls (pending CCs where user is a signatory)

Search results (when query entered — cross-type search):
  Results grouped by type with type header
  Each row: icon + name/title + status badge + dept badge

On select: closes popover, reference card preview appears above input, replaces any previous reference.
One reference per message maximum — selecting a new one replaces the old.
```

---

**Step 5 — Server Actions for Messaging**

**`sendMessage(conversationId, body, mentions[], referenceType?, referenceId?, replyToId?)`**
```typescript
// Verify: session, is_active, is member of conversationId
// Validate: body is not empty after trim, max 2000 chars
// Validate: if referenceId provided, referenceType must also be provided
// Parse @mentions from body: extract [name] tokens, resolve to user_ids where user is member
// Insert messages row
// DB triggers handle: update_conversation_last_message + notify_message_pulse
// Update own last_read_at: UPDATE conversation_members SET last_read_at = now()
//   WHERE conversation_id = X AND user_id = auth.uid()
// Return inserted message row
```

**`editMessage(messageId, newBody)`**
```typescript
// Verify: session, is_active, sender_id = auth.uid()
// Check edit window: IF now() - created_at > interval '15 minutes': throw 'Edit window closed'
// Update: body = newBody, is_edited = true, edited_at = now()
// Do NOT re-parse mentions or re-trigger pulse notifications on edit
```

**`deleteMessage(messageId)`**
```typescript
// Verify: session, is_active, sender_id = auth.uid()
// Soft-delete: UPDATE messages SET deleted_at = now() WHERE id = messageId AND sender_id = auth.uid()
// DO NOT update conversations.last_message_body — leave as is
```

**`createConversation(type, memberIds[], name?)`**
```typescript
// Validate: type = 'direct' requires exactly 1 other member_id
//           type = 'group' requires name (not empty) and at least 2 other member_ids
// For direct: check if a direct conversation already exists between these two users
//   If yes: return existing conversation_id (do not create duplicate)
//   If no: create new
// INSERT conversations row
// INSERT conversation_members for all members (including creator)
...
// WHERE conversation_id = X AND user_id = auth.uid()
// Called whenever user opens a conversation and when they send a message
```

**`updateNotifySetting(conversationId, setting)`**
```typescript
// Validate: setting IN ('all','mentions_only','muted')
// Validate: conversation type = 'group' (cannot mute direct conversations)
// UPDATE conversation_members SET notify_setting = setting
// WHERE conversation_id = X AND user_id = auth.uid()
```

**`leaveGroup(conversationId)`**
```typescript
// Verify: conversation type = 'group' (cannot leave a direct conversation)
// DELETE FROM conversation_members WHERE conversation_id = X AND user_id = auth.uid()
// If leaving user was the only remaining member: set conversations.is_archived = true
```

---

**Step 6 — `<NewConversationModal />`**

Shadcn Dialog, max-w-md.

Header: "New Conversation"

Type toggle (two card buttons, same style as onboarding role cards):
- `MessageSquare` icon "Direct Message" — one person
- `Users` icon "Group Conversation" — multiple people

People search (shown for both types):
```typescript
// Search query (debounced 300ms, min 2 chars)
supabase.from('profiles')
  ...
  .limit(10)
```
Results: list of `<UserSearchRow />` items (avatar + name + dept badge + role badge). Click to select.

Selected members shown as removable avatar chips below the search input.

Group name input (group type only, appears after at least 1 member selected):
Label "Group name" — required.

Footer:
- Direct: "Start Conversation" primary — creates and navigates to conversation.
- Group: "Create Group" primary — creates and navigates.
- Cancel ghost.

---

**Step 7 — `/messages/[id]/page.tsx`**

On mobile (< 768px): full-page conversation view with a back arrow to the list.
On desktop: this route redirects to `/messages` and sets the active conversation in Zustand store `useMessagesStore`.
...
```
The `/messages/[id]` page on desktop just calls `setActive(id)` and renders `/messages/page.tsx`. The conversation thread renders in the right panel driven by the store. This prevents a full page reload when switching conversations on desktop.

---

**Phase 13 Checklist:**
- [ ] Migration `014_messaging.sql` runs clean with zero errors
- [ ] RLS enabled on all three messaging tables
- [ ] Conversations SELECT: user can only query conversations they are a member of (verify by attempting direct API query for another user's conversation — must return empty)
- [ ] Messages SELECT: user can only query messages from their conversations (same test)
- [ ] Limited profiles SELECT policy works — people search returns active users, does not expose signature_url or sensitive fields
- [ ] Full-text search index exists on `messages.body_search` (verify in Supabase index list)
- [ ] Performance index exists on `messages(conversation_id, created_at DESC)`
- [ ] "Messages" nav item appears in sidebar with unread badge
- [ ] Unread badge count is accurate (test: send message from User A, verify User B sees correct count)
- [ ] Conversation list loads ordered by most recent message
- [ ] Creating a direct conversation with User B when one already exists returns the existing conversation (no duplicates)
- [ ] Group conversation requires a name and at least 2 other members
- [ ] Messages send correctly and appear in real-time for all members (test with two browser sessions)
- [ ] Typing indicator appears when the other user is typing (within ~1 second)
- [ ] Typing indicator disappears when they stop typing (within 2-3 seconds)
- [ ] @mention suggestions appear when typing `@` — filtered by conversation members
- [ ] @mention is highlighted in rendered message — current user's own mentions are highlighted brighter
- [ ] Mentioned user receives a Pulse notification even if conversation is set to 'mentions_only'
- [ ] Muted conversation sends no Pulse notification for regular messages
- [ ] Reference picker opens correctly, shows recent SOPs/equipment/open CCs
- [ ] Reference search returns results across types
- [ ] Selected reference embeds as a card in the sent message
- [ ] Reference card "Open" button opens the correct record in the correct module
- [ ] Superseded/inactive referenced records still render the card (no 404) with current status shown
- [ ] Cross-dept SOP referenced in a message: clicking "Open" follows the same visibility rules as the Library (read-only badge if not own dept)
- [ ] Reply quote renders correctly above the replying message
- [ ] Clicking a reply quote scrolls to the original message and briefly highlights it
- [ ] Edit works within 15 minutes — shows "edited" label
- [ ] Edit rejected after 15 minutes with clear error message
- [ ] Delete soft-deletes message — body replaced with "This message was deleted." Row stays in DB
- [ ] Infinite scroll loads older messages correctly (send 60+ messages, scroll to top)
- [ ] Date separators appear between messages from different days
- [ ] Mark-as-read: opening a conversation marks it read, unread badge clears
- [ ] Notify setting change (mute/unmute) updates immediately and persists on refresh
- [ ] Leave group removes user from conversation, conversation no longer appears in their list
- [ ] Last remaining member leaving archives the conversation
- [ ] Pulse `type='message'` item deep-links correctly into the conversation
- [ ] Mobile: conversation list and thread are separate full-page views with back navigation
- [ ] `npm run build` passes with zero TypeScript errors

**Commit:** `phase-13: messaging — conversations, group chat, dm, mentions, references, typing indicators, pulse integration`
...
## ADDITIONAL NEVER-DO LIST ENTRIES (Messaging)

These rows are added to the QUICK REFERENCE — NEVER DO LIST:

| Never Do This | Do This Instead |
|---------------|-----------------|
| Store chat messages in `pulse_items` | Messages live in the `messages` table. `pulse_items` only stores the notification that a message arrived (`type = 'message'`). |
| Write chat messages to `audit_log` | Chat is operational communication, not compliance records. Never log message content to audit_log. |
| Hard-delete a message | Soft-delete only: set `messages.deleted_at = now()`. The row stays. Body is hidden client-side. |
| Allow editing a message after 15 minutes | Server action checks `now() - created_at > interval '15 minutes'` and rejects with exception. UI disables edit button after 15 min but the server is the enforcement. |
| Create a duplicate direct conversation | `createConversation()` checks for an existing direct conversation between the two users first. If found, return existing id. Never create two direct conversations between the same pair. |
| Allow muting a direct message conversation | `notify_setting` only applies to group conversations. Direct messages always deliver a Pulse notification. |
| Skip the full-text search GIN index on messages | `CREATE INDEX messages_body_search_idx ON messages USING GIN(body_search)` must be in the initial migration. Never retrofit this on a populated table. |
| Build the typing indicator with DB rows | Typing indicator uses Supabase Realtime presence channels only. No database writes. No `pulse_items`. No `messages` rows. Presence is ephemeral. |
| Expose sensitive profile fields in the people-search query | People-search SELECT must only return: id, full_name, avatar_url, department, role. Never include signature_url, notification_prefs, or is_admin in messaging queries. |
| Allow a user to query another user's conversations | The conversations RLS policy must join through conversation_members. No admin override bypasses this. Chat is personal space. |
| Put a file upload option in the message input | File uploads are not supported. Show: "File uploads are coming soon. Use the reference button to link an existing document from the Library." |
| Hard-delete a conversation | Set `is_archived = true`. The row and all messages stay in the DB forever. |
