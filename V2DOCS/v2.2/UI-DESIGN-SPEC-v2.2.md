...
> **Version 2.1** | Companion to BUILD.md v2.2
...
## PART 19 — MESSAGING MODULE

### 19.1 Sidebar Nav Update

```
Messages nav item (between Equipment and Calendar):
  Icon:   MessageSquare (Lucide) 18px
  Label:  "Messages"
  Route:  /messages

Unread badge (when total unread > 0):
  Position: absolute top-0.5 right-1
  Size: min 16px h-4, rounded-full
  Background: bg-brand-teal     ← teal, not red (messages are not urgent system alerts)
  Text: 10px white font-700
  Content: count if ≤9, "9+" if >9
  Transition: scale-in 150ms when count changes from 0 to >0
              scale-out 150ms when count returns to 0

Active state: same as all nav items — bg-blue-50, border-l-[3px] border-brand-teal
```

---

### 19.2 Pulse — `type = 'message'` Item Renderer

```
Icon circle: bg-teal-100 icon=MessageSquare text-brand-teal 16px

Layout (same base as all Pulse items):
  Title: sender's full name — 13px/600 text-primary
  Body:  message preview, 12px text-secondary, line-clamp-1
         If message had a reference: "[Name]: [ref icon] [record title]"
         If message was a mention: "[Name] mentioned you: [preview]"
  Footer: conversation name (if group) in 11px text-muted · time-ago

Action: clicking the item opens /messages/[conversationId] and marks read
        Deep link always lands on the specific conversation.

Mention variant (type='message', user was @mentioned):
  Icon circle: bg-brand-teal icon=AtSign text-white   ← stronger visual for mentions
  Title: "[Name] mentioned you" in bold
  Distinct from regular message notification so user can prioritise
```

---

### 19.3 Messages Page `/messages`

```
Layout: full main content area (ml-[240px] mr-[300px] mt-[48px])
        grid grid-cols-[280px_1fr] h-[calc(100vh-48px)]
        No padding wrapper — the two panels sit flush to the edges

─── LEFT PANEL (280px) ──────────────────────────────────
Position: h-full flex flex-col
Background: bg-white
Border-right: 1px border-border-default
...
Header (52px flex-shrink-0):
  ...
  Left: "Messages" 15px/600 text-primary
  Right: "New" button — secondary sm h-28 px-2 icon=Plus text-12

Search (flex-shrink-0):
  px-3 py-2 border-bottom 1px border-border-default/50
  Input: h-32 w-full bg-surface-raised rounded-md border border-border-default
         pl-8 text-13 placeholder "Search conversations..."
         Search icon 14px text-muted left-inset (absolute left-2.5)
  Filters client-side — no new query on type

Conversation list (flex-1 overflow-y-auto):
  No padding — items are full-width
...
Background:
  Default:  bg-white
  Hover:    bg-surface-raised
  Active:   bg-blue-50 border-l-[3px] border-brand-teal
Transition: background-color 100ms
Cursor: pointer

Layout: flex items-center gap-3

[Avatar zone — 40px flex-shrink-0]
  Direct: <Avatar md /> of the other person
  Group: <GroupAvatar members={[...]} />   (see 19.4)

[Content — flex-1 min-w-0]
  Row 1: flex items-baseline justify-between gap-2
    Name: 13px/600 text-primary truncate flex-1
      Direct: other person's full name
      Group: conversation.name
    Time: 11px text-muted flex-shrink-0
      If today: "10:42" (time only)
      If this week: "Mon" (day name)
      If older: "12 Mar" (date)
  
  Row 2: flex items-center justify-between gap-2 mt-0.5
    Preview: 12px text-secondary truncate flex-1
      Normal: last message preview (60 chars max)
      Unread: 12px/500 text-primary (slightly bolder to signal unread)
      "No messages yet": 12px text-muted italic
      If deleted: "This message was deleted." in text-muted italic
      If had reference: "[📎 SOP title]" — paperclip emoji prefix
    
    Right badges (flex-shrink-0 flex items-center gap-1):
      Unread count badge:
        rounded-full bg-brand-teal text-white text-10 font-700 px-1.5 min-w-[18px] text-center
        Shown only when unread > 0
      Muted icon: BellOff 11px text-muted (when notify_setting = 'muted')
...
─── EMPTY CONVERSATION LIST ──────────────────────────────
  ...
  <EmptyState
    ...
    action={{ label: "New Conversation", onClick: openNewConversationModal }}
  />
...
No conversation selected (desktop):
  ...
  <EmptyState
    ...
    description="Choose from the list on the left or start something new."
  />

Conversation selected:
  Renders <ConversationThread /> — see 19.5
```
...
```
Props:
  members: Array<{ id: string; full_name: string; avatar_url?: string }>
  size?: 'sm' | 'md'    default: 'md'

Renders 2–3 overlapping small avatars showing group membership at a glance.

Layout (md — 40px footprint):
  Position: relative w-[40px] h-[40px]
  
  2 members:
    Avatar 1: 26px, absolute top-0 left-0, ring-1 ring-white
    Avatar 2: 26px, absolute bottom-0 right-0, ring-1 ring-white
  
  3+ members:
    Avatar 1: 24px, absolute top-0 left-0, ring-1 ring-white
    Avatar 2: 24px, absolute top-0 right-0, ring-1 ring-white
    Count circle ("+N"): 20px, absolute bottom-0 left-1/2 -translate-x-1/2
      bg-slate-200 text-text-secondary text-9 font-700 rounded-full
      Shows remaining count: if 5 members, shows +3

(sm — 28px footprint): same layout, smaller sizes proportionally
```

---

### 19.5 `<ConversationThread />`

```
Layout: h-full flex flex-col bg-white

─── THREAD HEADER (52px flex-shrink-0) ──────────────────
border-bottom: 1px border-border-default
Padding: px-5 flex items-center gap-3

[Avatar or GroupAvatar — 36px]

[Info flex-1 min-w-0]
  Name: 14px/600 text-primary truncate
    Direct: other person's full name
    Group: conversation.name
  Subtitle (group only): 12px text-secondary truncate mt-0.5
    "[Member 1], [Member 2]" + if >3: ", +N others"

[Actions — flex gap-1 items-center flex-shrink-0]
  Group only — UserPlus icon button (ghost 32px square, icon 16px, aria-label "Add member")
  Group only — Settings icon button (ghost 32px square, icon 16px, aria-label "Conversation settings")
              Opens <ConversationSettingsSheet />
  All — MoreHorizontal icon button (ghost 32px square, icon 16px)
        Dropdown: Archive (if not archived) | Unarchive (if archived) | [separator] | Leave Group (group only)

─── TYPING INDICATOR (24px flex-shrink-0) ───────────────
Height: 24px
Padding: px-5
Always mounted — height is always 24px whether typing or not (prevents layout shift)

When someone is typing:
  flex items-center gap-2
  Animated three dots:
    Three 4px circles bg-text-secondary, opacity 0.4
    Each dot: animate-bounce with delay 0ms, 150ms, 300ms
    Whole animation loops while typing is active
  "[Name] is typing..." 12px text-secondary italic
  
When nobody is typing: empty div (invisible but holds space)
...
Loading state (initial fetch):
  8 skeleton messages of varying width:
    flex gap-3 px-0 py-2
    Avatar skeleton (32px circle animate-pulse bg-slate-200)
    Content:
      Name skeleton: w-24 h-3 bg-slate-200 rounded animate-pulse
      Body skeleton: w-[60–85%] h-4 bg-slate-200 rounded animate-pulse mt-1.5

Load more (scroll to top trigger):
  When user scrolls within 50px of the top: fetch next 50 messages
  While loading: spinner (Lucide Loader2 animate-spin 16px text-muted) centered at top

─── DATE SEPARATOR ──────────────────────────────────────
  flex items-center gap-3 py-3 mx-0
  Left rule: flex-1 h-px bg-border-default
  Label: text-11 text-muted whitespace-nowrap px-2
    Today: "Today"
    Yesterday: "Yesterday"
    This year: "Monday, 10 March"
    Older: "10 March 2025"
  Right rule: flex-1 h-px bg-border-default

─── MESSAGE GROUP ────────────────────────────────────────
Consecutive messages from the same sender within 2 minutes = one group.
A group has one avatar + name header, then body-only lines below.

Group wrapper: flex flex-col mt-3 (extra top margin between different senders)

First message in group:
  flex gap-3 items-start
  [Avatar sm 28px — flex-shrink-0 mt-0.5]
  [Content flex-1 min-w-0]
    Header row: flex items-baseline gap-2 mb-1
      Sender name: 13px/600 text-primary
      Timestamp: 11px text-muted
    [Message bubble — see below]

Subsequent messages in same group (within 2 min, same sender):
  flex gap-3 items-start
  [Spacer 28px flex-shrink-0 — empty, keeps alignment with first message]
  [Message bubble — no name/timestamp header]
  Hover: show timestamp (absolute right: -60px, 10px text-muted) on hover only

─── MESSAGE BUBBLE ───────────────────────────────────────
No bubble background by default — messages are inline text, not chat bubbles.
(This matches the Slack/Discord aesthetic — cleaner, more professional than WhatsApp bubbles)

Own messages: no visual difference in body text.
              The hover actions are the only indicator it is your message.

Reply quote (shown when reply_to_id is set, above the body):
  border-l-2 border-brand-teal bg-surface-raised rounded-r-md
  px-3 py-1.5 mb-1.5 cursor-pointer
  max-w-[480px]
  
  flex items-start gap-2
    CornerDownRight icon 12px text-brand-teal flex-shrink-0 mt-0.5
    Content:
      "[Sender name]" 11px/600 text-brand-teal
      "[Body preview, truncated 80 chars]" 12px text-secondary mt-0.5 line-clamp-2
  
  On click: scroll to original message. Brief highlight on target:
    bg-brand-teal/10 transition: background-color 0→visible (100ms) then fade out (800ms)

Body (rendered text):
  14px text-primary leading-[1.5] max-w-[680px]
  
  @mention tokens:
    Format in DB body: "@[full_name]" stored literally
    Render: parse body, wrap each @name in:
      bg-brand-teal/15 text-brand-teal rounded-sm px-1 py-0 text-13/500
      cursor-default  (not a link — just highlight)
    
    Own mention (user_id is in message.mentions AND matches current user):
      bg-brand-teal/30 text-brand-teal (brighter)

Edited label (if is_edited = true):
  Inline after body text: " (edited)" 11px text-muted italic

Reference card (if reference_type is set AND deleted_at is null):
  mt-2 max-w-[420px]
  ...
  Icon column (flex-shrink-0 mt-0.5):
    SOP:            FileText 18px text-brand-teal
    Equipment:      Cog 18px text-brand-teal
    Change Control: GitBranch 18px text-brand-teal
  
  Content (flex-1 min-w-0):
    Type label: 10px uppercase letter-spacing-wide text-muted mb-1
      "SOP" / "EQUIPMENT" / "CHANGE CONTROL"
    Title: 13px/600 text-primary truncate
    Meta row: flex gap-2 items-center mt-1.5 flex-wrap
      <StatusBadge size="sm" />
      Version (if SOP): DM Mono 11px text-muted bg-surface-raised px-1.5 rounded-sm
      <DeptBadge size="sm" />
  
  Open button (flex-shrink-0):
    ghost sm h-28 px-2 text-12 text-brand-blue hover:underline
    "Open →"
    On click: opens the record in the relevant module
              SOP → opens in Library tab viewer
              Equipment → navigates to /equipment/[id]
              Change Control → navigates to /change-control/[id]
  
  Superseded / Inactive / Archived state (record is no longer active):
    Entire card: opacity-70
    Top-right corner badge:
      bg-status-inactive-bg text-status-inactive border border-slate-200
      text-10 px-1.5 rounded-sm
      "Superseded" / "Inactive"
    "Open →" button still enabled — shows record with current status

Deleted message:
  "This message was deleted."
  12px text-muted italic
  No body, no reference card, no hover actions
  opacity-70

─── HOVER ACTION BAR ────────────────────────────────────
Appears on message row hover (group:hover trigger on the message container).
Position: absolute right-4 top-0 translate-y-[-50%]
Background: bg-white border border-border-default rounded-md shadow-sm
Padding: px-1.5 py-1
Display: flex items-center gap-0.5
Z-index: z-10

All messages (not deleted):
  Reply icon button:  CornerUpLeft 15px ghost h-28 w-28 aria-label="Reply"
                      On click: sets reply state in input bar
  Mention icon button: AtSign 15px ghost h-28 w-28 aria-label="Mention sender"
                       On click: inserts @[sender_name] into input

Own messages only (not deleted, not expired edit window):
  [separator: w-px h-4 bg-border-default mx-0.5]
  Pencil icon button: 15px ghost h-28 w-28 aria-label="Edit message"
                      On click: enters edit mode inline (see 19.6)
                      Disabled + tooltip "Edit window closed" after 15 minutes

Own messages only (not deleted):
  Trash icon button: 15px ghost h-28 w-28 text-red-400 hover:text-red-600
                     aria-label="Delete message"
                     On click: opens <ConfirmDialog /> destructive
                               "Delete this message? This cannot be undone."

─── INLINE EDIT MODE ─────────────────────────────────────
When edit is triggered:
  The message body area is replaced in-place with:
  Textarea (same width as body, auto-height): pre-filled with current body text
  bg-surface-raised border border-brand-teal rounded-md px-3 py-2 text-14
  
  Below textarea (flex gap-2 mt-1.5):
    "Esc to cancel" 11px text-muted
    [spacer]
    Cancel ghost sm h-28 text-12
    Save primary sm h-28 text-12 — calls editMessage() server action
  
  Escape key: cancels edit, restores original body display
  Enter (without Shift): saves. Shift+Enter: newline.
...
Reply preview (shown above input when replying to a message):
  bg-surface-raised border-l-2 border-brand-teal rounded-r-md
  px-3 py-2 mb-2
  flex items-center gap-3

  CornerDownRight icon 14px text-brand-teal flex-shrink-0
  Content flex-1 min-w-0:
    "Replying to [sender name]" 12px/600 text-brand-teal
    "[Body preview]" 12px text-secondary truncate mt-0.5
  X button (ghost 20px square):
    XCircle icon 14px text-muted hover:text-text-primary
    On click: clears reply state

Reference card preview (shown above input when reference selected):
  Same card style as in messages but compact height:
  flex items-center gap-3 bg-surface-raised border border-border-default rounded-md px-3 py-2 mb-2
  [icon 16px text-brand-teal]
  [Title 13px/600 text-primary truncate flex-1]
  [Status badge sm]
  [X button — removes reference before send]

@mention suggestions popover:
  Position: absolute bottom: 100% + 8px, left: 0
  min-width: 240px max-width: 300px
  bg-white border border-border-default rounded-lg shadow-lg
  max-h-[200px] overflow-y-auto
  
  Shows when user types @[2+ chars]
  
  Header: "Members" 10px uppercase text-muted px-3 py-2 border-b border-border-default
  
  Each user row: flex items-center gap-3 px-3 py-2.5 hover:bg-surface-raised cursor-pointer
    <Avatar sm 24px />
    Full name 13px/600 text-primary
    <DeptBadge size="sm" />
  
  Active row (keyboard nav): bg-blue-50
  Keyboard: ↑↓ to navigate, Enter/Tab to select, Esc to dismiss
  
  Empty (no matching members): "No members found" text-muted text-12 px-3 py-3

Input row: flex items-end gap-2

  Reference button:
    ghost h-36 w-36 flex-shrink-0
    Paperclip icon 18px text-muted hover:text-text-primary
    aria-label "Attach reference"
    On click: opens <ReferencePicker /> popover (see 19.7)
    Active state (reference selected): text-brand-teal bg-teal-50 rounded-md

  Textarea (flex-1):
    min-h-[36px] max-h-[120px] resize-none
    bg-surface-raised border border-border-default rounded-lg
    px-4 py-2 text-14 text-primary leading-relaxed
    placeholder: "Message [other person's name]..." (DM) or "Message [group name]..." (group)
    Focus: ring-2 ring-brand-teal border-brand-teal
    Auto-resize: grows with content up to max-h
    
    On Enter (no Shift): submit message
    On Shift+Enter: insert newline
    On @: trigger mention suggestions when followed by 2+ chars

  Send button:
    primary h-36 w-36 flex-shrink-0
    icon=Send 16px
    aria-label "Send message"
    Disabled: body is empty or whitespace-only → opacity-50 cursor-not-allowed
    Enabled: bg-brand-teal hover:bg-teal-600
    On click: calls sendMessage() server action, clears input, clears reply/reference state
```

---

### 19.6 `<ConversationSettingsSheet />`

```
Shadcn Sheet from right, w-[320px]
Only for group conversations. Not available for direct messages.

Header: "Conversation Settings"

─── RENAME ──────────────────────────────────────────────
"Conversation Name" label 13px/600 text-primary mb-1.5
Input pre-filled with current name + "Save" primary sm button inline
Validation: required, 1–60 chars. Save calls updateConversationName() server action.

─── NOTIFICATIONS ───────────────────────────────────────
"Notifications" 13px/600 text-primary mt-6 mb-2
Three radio options (styled as button group):
  All messages | @Mentions only | Muted
  Active: bg-brand-teal text-white
  Inactive: bg-white border border-border-default text-text-secondary
  On change: calls updateNotifySetting() server action immediately

─── MEMBERS ─────────────────────────────────────────────
"Members ([N])" 13px/600 text-primary mt-6 mb-2
Member list (max-h-[240px] overflow-y-auto):
  Each row: flex items-center gap-3 py-2
    <Avatar sm />
    Name 13px text-primary + role/dept badges sm
    Creator badge: "Creator" pill bg-brand-navy/10 text-brand-navy text-10 px-1.5 rounded-full
    Remove button (icon=X ghost 20px) — visible to conversation creator only, not on own row

"Add Member" button: full-width secondary sm icon=UserPlus mt-3
On click: opens people-search within the sheet
  Search input → results list → click to add
  Cannot add someone already in the conversation (they are filtered out of results)

─── DANGER ZONE ─────────────────────────────────────────
border-t border-border-default mt-6 pt-4

"Leave Conversation" button: full-width destructive variant
icon=LogOut text-14
On click: <ConfirmDialog> "Leave this group conversation? You will no longer receive messages."
Confirm calls leaveGroup() server action.
```

---

### 19.7 `<ReferencePicker />`

```
Shadcn Popover, anchored to the reference button in the input bar.
Width: 360px
Position: above the input bar (side: 'top', align: 'start')
Max-height: 400px
Background: bg-white
Border: 1px border-border-default
...
─── SEARCH INPUT ─────────────────────────────────────────
px-3 pt-3 pb-2 border-b border-border-default
Input: h-36 w-full bg-surface-raised rounded-md pl-8 text-13
       placeholder "Search SOPs, equipment, change controls..."
       Search icon 14px text-muted left-inset
       Auto-focused when popover opens

─── BROWSE MODE (search empty) ───────────────────────────
Three sections, each with a type header:

Section header:
  px-3 py-1.5 text-11 uppercase letter-spacing-wide text-muted
  bg-surface-raised/50 border-b border-border-default/50

Recent SOPs (last 5 from user's visible Library — Active, own dept first):
  [FileText 14px text-brand-teal] [SOP Number DM Mono 12px text-muted] [Title 13px text-primary truncate] [<DeptBadge sm>]

Recent Equipment (last 5 Active, own dept):
  [Cog 14px text-brand-teal] [Asset ID DM Mono 12px text-muted] [Name 13px text-primary truncate] [<DeptBadge sm>]

Open Change Controls (pending CCs where user is in required_signatories):
  [GitBranch 14px text-brand-teal] [CC ref DM Mono 12px text-muted] [SOP title 13px text-primary truncate] [<StatusBadge sm />]

─── SEARCH MODE (query ≥ 2 chars) ────────────────────────
Results grouped by type.
Loading: 3 skeleton rows per type.
No results: "No matching records" text-muted text-13 px-4 py-6 text-center.

Each result row:
  Height: 44px px-4 flex items-center gap-3 hover:bg-surface-raised cursor-pointer
  Same layout as browse mode rows above.

─── SELECTION ────────────────────────────────────────────
On row click:
  1. Close popover
  2. Set reference state: { type, id, title, status, department, version }
  3. Reference preview card appears above the input bar (see input bar spec)
  4. Reference button turns teal (active state)

If a reference is already selected and user opens the picker again:
  Current selection shown at top with a "Remove" option before the search/browse list.

One reference per message maximum.
```

---

### 19.8 `<NewConversationModal />`

```
Shadcn Dialog, max-w-md

Header: "New Conversation" 20px/700

─── TYPE SELECTOR ────────────────────────────────────────
grid grid-cols-2 gap-3 mb-6

Two cards (same style as onboarding role cards, 80px height):
  Direct Message card:
    Icon: MessageSquare 22px text-brand-blue
    Label: "Direct Message" 14px/600 text-primary mt-2
    Sub: "One-on-one" 12px text-secondary mt-0.5
  
  Group Conversation card:
    Icon: Users 22px text-brand-blue
    Label: "Group Chat" 14px/600 text-primary mt-2
    Sub: "Multiple people" 12px text-secondary mt-0.5

Selected card: bg-blue-50 border-2 border-brand-blue
Unselected: bg-white border-2 border-border-default hover:border-border-strong

─── PEOPLE SEARCH ────────────────────────────────────────
Label: "Add people" 13px/600 text-primary mb-1.5

Search input: h-36 full-width, placeholder "Search by name..."
              min 2 chars to trigger query
...
Results dropdown (below input):
  ...
  Each row: flex items-center gap-3 px-4 py-2.5 hover:bg-surface-raised cursor-pointer
    <Avatar sm />
    Full name 13px/600 text-primary
    Job title 12px text-secondary
    <DeptBadge size="sm" />
  
  Already-selected: greyed out with checkmark, not clickable
  
  Empty: "No active users found" text-muted text-12 px-4 py-3

Selected members (chips, below search):
  flex flex-wrap gap-2 mt-2 min-h-[32px]
  Each chip: flex items-center gap-1.5 bg-blue-50 border border-blue-200 rounded-full
             px-2 py-1
    <Avatar xs 18px />
    Name 12px/500 text-brand-blue
    × button (XCircle 12px text-blue-400 hover:text-blue-700) — removes from selection

─── GROUP NAME (group type only, shown when ≥1 member selected) ──
Label: "Group name" 13px/600 text-primary mt-4 mb-1.5
Input: h-36 full-width placeholder "e.g. QA + Engineering, Project Alpha..."
Validation: required, shown inline: "Group name is required" text-12 text-red-600 mt-1

─── EXISTING CONVERSATION WARNING (direct type) ──────────
Shown when selected person already has a direct conversation with the current user:
bg-amber-50 border border-amber-200 rounded-lg p-3 mt-3
MessageSquare 14px text-amber-600 + "You already have a conversation with [name]."
"Open it" text-brand-blue link → closes modal, navigates to existing conversation.
"Create anyway" is NOT offered — one DM per pair is enforced.

─── FOOTER ────────────────────────────────────────────────
flex justify-between items-center pt-4 border-t border-border-default mt-4

Left: Cancel ghost button

Right:
  Direct (1 member selected): "Start Conversation" primary
  Group (≥2 members + name): "Create Group" primary
  Neither condition met: button disabled with appropriate tooltip
    Direct, no member: "Select someone to message"
    Group, no name: "Add a group name"
    Group, <2 members: "Add at least 2 people"
```

---

### 19.9 Mobile Messaging Layout (< 768px)

```
On mobile, the two-panel layout is replaced with full-screen navigation:

/messages — shows ONLY the conversation list (full width)
  Same list rendering as desktop left panel
  Each row tap navigates to /messages/[id]

/messages/[id] — shows ONLY the conversation thread (full width)
  Back button in a mini-header above the thread header:
    ← "Messages" — ChevronLeft 18px + "Messages" text-14 text-brand-blue
    Navigates back to /messages list
  
  Thread header: same as desktop but without the back area
  Message area: full viewport height minus header + input bar
  Input bar: fixed to bottom of screen

Keyboard handling on mobile:
  When soft keyboard appears, the input bar stays above the keyboard.
  Message area height shrinks to accommodate.
  Use: height: calc(100dvh - [header] - [keyboard]) with dvh units.

Touch targets: all message hover actions become a long-press menu on mobile.
  Long-press on a message → bottom sheet appears with: Reply | Mention | Edit | Delete
  Shadcn Sheet from bottom, list of large touch-friendly option rows (48px each)
```

---

### 19.10 Empty States — Messaging

```
No conversations (/messages, list is empty):
  Icon: MessageSquare
  Title: "No conversations yet"
  Description: "Start a direct message or create a group to collaborate with your team."
  Action: "New Conversation"

No conversation selected (right panel, desktop):
  Icon: MessageSquare  
  Title: "Select a conversation"
  Description: "Choose from the list or start something new."
  (No action button — the "New" button in the left panel header serves this)

No messages in open conversation:
  Centered vertically in message area
  Icon: MessageSquare 40px text-muted
  "[Name]" 16px/600 text-primary mt-3 (person's name or group name)
  "This is the beginning of your conversation." 13px text-secondary mt-1
  (No action — just informational)

No search results (conversation list search):
  Inline in list, not a full empty state:
  "No conversations match "[query]"" 13px text-muted px-4 py-6 text-center

No reference picker results:
  "No matching records" 13px text-muted px-4 py-6 text-center
  "Try a different search term." 12px text-muted mt-1
```

---

### 19.11 Loading States — Messaging

```
/messages page load (loading.tsx):
  Left panel: 5 conversation row skeletons
    Each: flex gap-3 items-center px-4 py-3 h-[68px]
    Avatar skeleton: 40px circle animate-pulse bg-slate-200
    Content:
      Name skeleton: w-32 h-3 bg-slate-200 rounded animate-pulse
      Preview skeleton: w-48 h-2.5 bg-slate-200 rounded animate-pulse mt-1.5
  
  Right panel: 6 message skeletons (alternating left/right to suggest a conversation)
    See 19.5 loading state spec above

Thread initial load: see 19.5 message area loading state
Load more (scroll to top): Loader2 16px animate-spin text-muted centered for 300ms

Reference picker search: 3 skeleton rows per type section while fetching

People search results: 3 skeleton user rows while fetching
```

---

### 19.12 Animation — Messaging Specific

```
New message appears (Realtime INSERT):
  The new message row: opacity 0→1, translateY 8px→0
  Duration: 200ms ease-out
  Do NOT use AnimatedList here (that is for the Pulse sidebar)
  Use a simple CSS animation on the newly appended message div

Sending own message:
  Message appears immediately (optimistic update) with opacity-70
  On server confirmation: opacity 70→100, no movement
  On server error: message row shakes (translateX: 0→-4px→4px→0, 200ms)
                   and shows inline error badge: "Failed to send. Retry?"

Unread badge on sidebar:
  When count goes 0→1: scale 0→1 (spring, 250ms)
  When count increases: brief scale pulse (1→1.15→1, 200ms)
  When count goes to 0 (all read): scale 1→0 (150ms ease-in)

Typing indicator dots:
  CSS animation: @keyframes bounce (translateY 0→-4px→0, 400ms ease-in-out)
  Dot 1: animation-delay 0ms
  Dot 2: animation-delay 150ms
  Dot 3: animation-delay 300ms
  All three loop while typing is active
  Appear/disappear: opacity 0→1 (150ms) / opacity 1→0 (150ms)

Mention suggestions popover:
  Appear: opacity 0→1, translateY 4px→0, 150ms ease-out
  Dismiss: opacity 1→0, 100ms ease-in

Reference card (embed in message):
  On first appear (after sending): opacity 0→1, max-height 0→auto, 200ms ease-out
  Subtle: does not distract from message text

ConversationSettingsSheet:
  Shadcn Sheet from right — 300ms ease-out (standard sheet animation)

Reply quote highlight (clicking a quote to scroll to original):
  Target message: bg-brand-teal/10 applied for 800ms then fades to transparent
  transition: background-color 100ms ease-out (apply), 600ms ease-in (remove)
```

---

*End of UI Design Spec v2.2 — SOP-Guard Pro*
*Companion to BUILD.md v2.2*
*Every component specified. No state left undefined. Messaging module fully specced.*
